 ...
 * ManageGridJson.js
 * 
 * com.tws.script.extGridJson.gbag.seizeRes.ManageGridJson.js
 * 已处置列表
 * 
 */
function getJsonData(){
	...
	sql1="SELECT T4.uuid,T4.casenumber,T4.suspect,CONCAT(T2.casenumber,'(',T2.casename,')') AS casename,T2.`casetype`,T2.`means` datatype," +
			...
			"k1.`name` AUDITUSERNAME,case T4.state when '3' then '已归还' when '4' then '已销毁 ' when '5' then '已上缴国库' when '6' then '已移交' end as statename " +
			...
			"LEFT JOIN k_dic T5 ON T5.`ctype`='扣押物品处置状态' AND t4.`state`=t5.`Value` " +
			"WHERE t4.state+0>2 AND T1.organid_='"+organId+"' GROUP BY T4.`uuid`";
