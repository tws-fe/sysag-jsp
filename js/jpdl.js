...
var defaultClass = "com.szk.impl.process.base.Assignment" ;
var defaultHandlerClass = "com.szk.impl.process.base.BaseNodeHandler" ;
		...
		if(D) {
			...
			for(var $=0;$<_.selections.length;$++) {
				...
				if(A.getId()==D.getId()&&A.el.tagName!="line"&&A.el.tagName!="polyline")if(_.activePalette!="transition-straight"&&_.activePalette!="transition-broken") {
					...
					document.getElementById("isSignNode").style.display = "none";
					if(D.nodeName == "task") {
						...
						//document.getElementById("isSignNode").checked = false;
						document.getElementById("isSignNode").style.display = "";
						document.getElementById("isSignNode").value = "";
						if(D.buttonPopedom) {
							var tt = D.buttonPopedom;
							if(tt.indexOf("会签节点")>-1){
								document.getElementById("isSignNode").value = "会签节点";
								tt = tt.replace("会签节点","");
								if(tt == ""){
									Ext.getCmp("buttonPopedom").clear();
								}else{
									Ext.getCmp("buttonPopedom").setRealValue(tt.substring(1));
								}
							}else if(tt.indexOf("级联审批")>-1){
								document.getElementById("isSignNode").value = "级联审批";
								tt = tt.replace("级联审批","");
								if(tt == ""){
									Ext.getCmp("buttonPopedom").clear();
								}else{
							...
							}else if(tt.indexOf("并联审批")>-1){
								document.getElementById("isSignNode").value = "并联审批";
								tt = tt.replace("并联审批","");
								if(tt == ""){
									Ext.getCmp("buttonPopedom").clear();
								}else{
								...
								Ext.getCmp("buttonPopedom").setRealValue(D.buttonPopedom);
							}
						}else {
							Ext.getCmp("buttonPopedom").clear();
						}
						...
						if(dProcess != ""){
							Ext.getCmp("dProcess").setRealValue(dProcess.substring(1));
						}else{
							Ext.getCmp("dProcess").clear();
						}
						
						if(D.level != undefined && D.level != ""){
							Ext.getCmp("level").setRealValue(D.level);
						}else{
							Ext.getCmp("level").clear();
						}
						
						//alert(D.level + "|" + D.dLetter);
						...
						if(rightType == "自定义权限") {
							...
							if(strategy) {
								Ext.getCmp("strategy").setRealValue(strategy);
							}else {
								Ext.getCmp("strategy").clear();	
							}
...
Jpdl.ActivityMap={
	activityBasePath:"jpdl/img/activities/48/",
...
Jpdl.xml.AbstractWrapper.prototype={
	...
	//	else $.push((_.x-5),",",(_.y-5),",",(_.w-10),",",(_.h-10),"'")
		...
		if(_.buttonPopedom)
			$.push(" candidate-groups='",_.buttonPopedom,"' ");
	...
	appendBody:function (_) {
		...
		if(node.department || node.role || node.user || node.strategy) {
			...
			if(node.strategy)
				_.push("       <field name='strategy'><string value='" + node.strategy + "'/></field> \n") ;
			
			_.push("        </assignment-handler> \n") ;
		}
		
		//alert(node.sTemplet + "|" + node.dTemplet);
		
		var msg = "",level = "";
		if(node.sLetter) {
			msg += "sLetter"  ;
			if(node.sTemplet != undefined && node.sTemplet != "" && node.sTemplet != "undefined"){
				msg += "`" + node.sTemplet;
			}
			msg += ",";
		}
		...
		if(node.dLetter) {
			msg += "dLetter" ;
			if(node.dTemplet != undefined && node.dTemplet != "" && node.dTemplet != "undefined"){
				msg += "`" + node.dTemplet;
			}
			msg += ",";
		}
		...
		_.push("       	 </field> \n") ;
		_.push("      	 <field name='level'> \n") ;
		_.push("       		<string value='"+level+"'/>   \n") ;
		_.push("       	 </field> \n") ;
...
Jpdl.xml.JpdlParse.prototype={
	...
	parseNode:function($model) {
		...
		for(var i=0;i<nodes.length;i++) {
			...
			var buttonPopedom = node.getAttribute("candidate-groups") ;
			...
			if(buttonPopedom) editNode.buttonPopedom = buttonPopedom;
			...
			for(var j=0;j<childs.length;j++) {
				...
				if(tagName == "assignment-handler" && className == defaultClass) {
					...
					for(var k=0;k<fields.length;k++) {
							...
							if(msg != null && msg != "") {
								if(msg.indexOf("sLetter") > -1) {
									...
									for(var _i=0; _i< _t.length;_i++){
										if(_t[_i].indexOf("sLetter") > -1){
											var _tt = _t[_i].split("`");
											if(_tt.length == 2) editNode.sTemplet = _tt[1];
											break;
										}
									}
								} 
								...
								if(msg.indexOf("dLetter") > -1) {
									...
									for(var _i=0; _i< _t.length;_i++){
										if(_t[_i].indexOf("dLetter") > -1){
											var _tt = _t[_i].split("`");
											if(_tt.length == 2) editNode.dTemplet = _tt[1];
											break;
										}
									}
								} 
								...
								if(msg.indexOf("dWx") > -1) {
									//给办理人发送手机短信
									dProcess += ",Wx";
									editNode.dWx = true ;
								} 
								if(msg.indexOf("dEmail") > -1) {
									//给办理人发送手机短信
									dProcess += ",Email";
									editNode.dEmail = true ;
								} 
								if(msg.indexOf("dAE") > -1) {
									//给办理人发送手机短信
									dProcess += ",AE";
									editNode.dAE = true ;
								}
						...
						if(fieldName == "level") {
							var level = field.childNodes[0].getAttribute("value") ;
							editNode.level = level;
						}
					}
