...
//if (oblog_bIsNC){
//document.write('<iframe width="260" height="165" id="colourPalette" src="../AS_INCLUDE/images/nc_selcolor.htm" style="visibility:hidden; position: absolute; left: 0px; top: 0px;" frameborder="0" scrolling="no" ></iframe>');
//}
...
function oblog_foreColor()
{
	...
	if (oblog_bIsIE5){
		var arr = showModalDialog("/AuditSystem/AS_INCLUDE/images/selcolor.html", "", "dialogWidth:18.5em; dialogHeight:17.5em; status:0; help:0");
		if (arr != null) FormatText('forecolor', arr);
		else IframeID.focus();
	}else
		{
		FormatText('forecolor', '');
		//var arr = openEditScript('../AS_INCLUDE/images/nc_selcolor.htm',250,100)}
		}
}
...
function OpenSmiley()
{
	var arr = showModalDialog("../AS_INCLUDE/images/smiley.htm", "", "dialogWidth:60em; dialogHeight:15.5em; status:0; help:0");
	if (arr != null){
		var ss;
		ss=arr.split("*")
		path=ss[0];
...
{
	var arr = showModalDialog("../AS_INCLUDE/images/swf.htm", "", "dialogWidth:30em; dialogHeight:10em; status:0; help:0");
	if (arr != null){
		var ss;
		ss=arr.split("*")
		path=ss[0];
...
{
	var arr = showModalDialog("../AS_INCLUDE/images/wmv.htm", "", "dialogWidth:30em; dialogHeight:13.5em; status:0; help:0");
...
{
	var arr = showModalDialog("../AS_INCLUDE/images/rm.htm", "", "dialogWidth:30em; dialogHeight:13.5em; status:0; help:0");
...
function oblog_UserDialog(what)
{
	...
	if (what == "CreateLink") {
		if (oblog_bIsNC)
		{
			insertLink = prompt("����д����t�ӵ�ַ��Ϣ��", "http://");			
...
function oblog_code()
{
	oblog_specialtype("<div class=HtmlCode style='cursor: pointer'; title='������иô��룡' onclick=\"preWin=window.open('','','');preWin.document.open();preWin.document.write(this.innerText);preWin.document.close();\">","</div>");	
...
{
	var arr = showModalDialog("../AS_INCLUDE/images/replace.html", "", "dialogWidth:16.5em; dialogHeight:13em; status:0; help:0");
	if (arr != null){
		var ss;
		ss = arr.split("*")
		a = ss[0];
...
{
	var arr = showModalDialog("../AS_INCLUDE/images/specialchar.html", "","dialogWidth:25em; dialogHeight:15em; status:0; help:0");
...
function oblog_fortable()
{
	if (!oblog_validateMode())	return;
	IframeID.focus();
	var arr = showModalDialog("../AS_INCLUDE/images/table.html", window, "dialogWidth:22em; dialogHeight:18.5em; status:0; help:0;scroll:no;");
	if (arr)
	{
...
function oblog_DeleteRow()
{
	...
	{
		parentTable.parentNode.removeChild(parentTable); // parentTable.removeNode(true);
...
{
	var arr = showModalDialog("/AuditSystem/AS_INCLUDE/images/emot.htm", "", "dialogWidth:20em; dialogHeight:12em; status:0; help:0");
...
{
if (oblog_bIsIE5){	
	var arr=showModalDialog("../AS_INCLUDE/images/img.htm",window, "dialogWidth:26em; dialogHeight:20.5em; status:0; help:0");
	IframeID.focus();
	if (arr != null)
	{
		//content=oblog_Composition.document.body.innerHTML;
...
{
if (oblog_bIsIE5){		
	var arr=showModalDialog("/AuditSystem/AS_INCLUDE/images/link.htm",window, "dialogWidth:23em; dialogHeight:11em; status:0; help:0");
	IframeID.focus();
	if (arr != null)
	{		
		oblog_InsertSymbol(arr);
...
{
	var arr = showModalDialog("../AS_INCLUDE/images/file.htm", "", "dialogWidth:23em; dialogHeight:11em; status:0; help:0");
