...
var DEFAULT_REFRESHURL= TEMP_SYSTEM_WEB_ROOT + "/hint.do";
var NOSHOW_HINT;
var _myHistory;
  ...
  try{
	...
	*/
	...
	try{
		...
		suburl=DEFAULT_REFRESHURL+"?method=list&autoid="+CURRENT_SELECT_OBJ.autoid;
		if (pk1!=""){
			suburl+="&pk1="+pk1;
		};
			...
			if(qqq) {
				if (qqq.tagName.toUpperCase()=="INPUT" && qqq.type.toUpperCase()=="FILE" ){
					...
					if (qqq.value!=null && qqq.value!=""){
						...
						//alert(_filename);
						suburl += "&refer="+encodeURI(_filename);
					}
				}else{
					//非file类型的INPUT等
					if (qqq.value!=null && qqq.value!=""){
						suburl += "&refer="+encodeURI(qqq.value);
					}
				}
			} else {
				suburl += "&refer=" + encodeURI(CURRENT_SELECT_OBJ.refer);
			}
		...
		if (CURRENT_SELECT_OBJ.refer1){
			...
			if(qqq1) {
				if (qqq1.value!=null && qqq1.value!=""){
					suburl += "&refer1="+encodeURI(qqq1.value);
				}
			} else {
				suburl += "&refer1=" + encodeURI(CURRENT_SELECT_OBJ.refer1);
			}
		...
		if (CURRENT_SELECT_OBJ.refer2){
			...
			if(qqq2) {
				if (qqq2.value!=null && qqq2.value!=""){
					suburl += "&refer2="+encodeURI(qqq2.value);
				}
			} else {
				suburl += "&refer2=" + encodeURI(CURRENT_SELECT_OBJ.refer2);
			}
...
function _doClear() {
	try {
		//清空文本框值
		//alert(CURRENT_SELECT_OBJ.id+"|"+CURRENT_SELECT_OBJ.clone+"|"+CURRENT_SELECT_OBJ.cloneId+"|"+CURRENT_SELECT_OBJ.value);
		...
		if (oInputs(i).autoid && !oInputs(i).clone){
			...
			*/
			...
			if (oInputs(i).value != null && oInputs(i).value !=""
				...
				//var url="/audit/hint.do?method=oldselect&checkmode=1&autoid="+oInputs(i).autoid+"&pk1=" + oInputs(i).value;
				var url=DEFAULT_REFRESHURL + "?method=list&checkmode=1&autoid="+oInputs(i).autoid+"&pk1=" + oInputs(i).value;
				//进行后台校验
				...
				var oBao = new XMLHttpRequest(); // oBao.asynchronous=false;
