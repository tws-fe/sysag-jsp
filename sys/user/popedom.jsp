...
function ext_init() {
	...
	var popedemTreeLoader = new Tree.TreeLoader({
        dataUrl:'${pageContext.request.contextPath}/user.do?method=popedom&userId=${userId}'
    });
     ...
     var treeTbar=new Ext.Toolbar({
 		items:[{ 
 			...
 			handler:function(){
 				 ...
 				 document.thisForm.action="${pageContext.request.contextPath}/userNew.do?method=popedomSave";	
 		...
 		},'-','<div style="margin-left:50px;color:#ff6600;">人员名称：${userName}</div>'
	    ...
	    buttons:[{
            text:'保存',
          	handler:function() {
				...
				jQuery.ajax({
					url : MATECH_SYSTEM_WEB_ROOT + '/role.do?method=popedomOfDepartmentSave',
					type : 'POST',
					data : {
						menuid : node_menu_current.id,
						user_or_role_id : document.getElementById('userId').value,
...
function tree_depart_init(node_menu){
	...
	var data = new Ext.tree.TreeLoader({
		url:'${pageContext.request.contextPath}/department.do?method=getVisibleDepartmentTree&userbool=true&user_or_role=user'
	});
	...
	data.on('beforeload',function(treeLoader,node){
		...
		this.baseParams.roleid = document.getElementById("userId").value,
