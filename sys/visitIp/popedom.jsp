...
function ext_init() {
	...
	var popedemTreeLoader = new Tree.TreeLoader({
        dataUrl:'${pageContext.request.contextPath}/visitIp.do?method=popedom&roleId=${roleId}'
    });
    ...
    tree_right.on('append', function(tree, node){
     ...
     var treeTbar = new Ext.Toolbar({
 		items:[{ 
 			...
 			handler:function(){
 				...
 				if("${sessionScope.userSession.userLoginId}" !="admin"){
 				 ...
 				 document.thisForm.action="${pageContext.request.contextPath}/visitIp.do?method=popedomSave";	
 		...
 		},'-','<div style="margin-left:50px;color:#ff6600;">IP名称：${loginIp}</div>'
	    ...
	    buttons:[{
            text:'保存',
          	handler:function() {
				...
				jQuery.ajax({
					url : MATECH_SYSTEM_WEB_ROOT + '/visitIp.do?method=popedomOfDepartmentSave',
					type : 'POST',
					data : {
						menuid : node_menu_current.id,
						user_or_ip_id : document.getElementById('roleId').value,
...
function tree_depart_init(node_menu){
	...
	var data = new Ext.tree.TreeLoader({
		url:'${pageContext.request.contextPath}/department.do?method=getVisibleDepartmentTree&userbool=true&user_or_role=loginIp'
	});
