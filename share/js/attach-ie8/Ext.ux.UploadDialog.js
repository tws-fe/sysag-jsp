...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	fillToolbar : function() {
		...
		if (this.isView == false) {
			tb.addItem(new Ext.ux.UploadDialog.TBBrowseButton({
	...
	isPermittedFileType : function(filename) {
		...
		if(attachlimitupload.indexOf(this.getFileExtension(filename))!=-1){
...
//						.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
		...
		Ext.MessageBox.confirm('确认删除', '确认要删除所选附件吗?', function(btn) {
					if (btn == 'yes') {
						...
						if (!Ext.isEmpty(delIds)) {
							Ext.Ajax.request({
										url : MATECH_SYSTEM_WEB_ROOT + "/common.do?method=attachRemove",
										method : 'POST',
	...
	updateRecordState : function(data) {
		if ("success" in data.response && data.response.success) {
			...
			data.record.set('id', data.response.attachId);
			// }
		} else {
			data.record.set("state",
	...
	onAjaxSuccess : function(response, options) {
		...
		try {
			var text = response.responseText;//replaceHtml(response.responseText);// 替换<pre></pre>
			...
			if (this.hiddenFieldId) {
				...
				if (Ext.isEmpty(v))
					Ext.getCmp(this.hiddenFieldId)
							.setValue(json_response.attachId);
				else
					Ext.getCmp(this.hiddenFieldId).setValue(v + ","
							+ json_response.attachId);
			}
		} catch (e) {
			alert("上传的附件大小不能超过"+SYSTEM_FILE_SIZE_MAX);
		}
		var data = {
	...
	isNotEmptyQueue : function() {
		var b = true;
		try{
			b = this.grid_panel.getStore().getCount() > 0
		}catch(e){}
		return b
	},
...
p.i18n = {
	...
	progress_waiting_text : "请选择文件后点开始上传...",
	progress_uploading_text : "上传中: {0} 的 {1} 文件集合成功",
	error_msgbox_title : "提示",
	permitted_extensions_join_str : ",",
	err_file_type_not_permitted : "该类型的文件已被限制上传！",
