...
Ext.matech.grid.GridPanel = Ext.extend(Ext.grid.GridPanel,{
	...
	initHideDom:function(){
		...
		var qryhtml='<div id="customQry_'+this.tableId+'" style="display: none;"><br/>'+
			'<table class="qryTb" style="margin: auto;" align="center"><tr><th width="5%"> <a href="javascript:;" onclick="matech.addQuery(\''+this.tableId+'\');"> '+
			'<img src="'+TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/add.gif" /></a> </th> <th width=20%>列名称</th><th width=8%>条件</th><th width=30%>值</th>'+
		...
		var cfghtml='<div id="gridConfig_'+this.tableId+'" style="display: none;"><br/>'+
		 			'<table class="qryTb" style="margin: auto;" align="center"> <tr><th width="5%">选</th><th width="20%" align="left">描述</th>	</tr>'+
		 	...
		 	if(self.isShowBbar){
				...
				if(self.isShowRefresh){
					_bbar_items=_bbar_items.concat(['-',{
													text:'刷新',
													icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/refresh.gif',
													handler:function(){
				...
				if(self.isShowQuery){
					_bbar_items=_bbar_items.concat(['-',{
													text:'筛选',
													icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/sqlQuery.png',
													handler:function(){
				...
				if(self.isShowExport){
					_bbar_items=_bbar_items.concat(['-',{
													text:'导出',
													icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/excel.gif',
													handler:function(){
				...
				if(self.isShowClearState){
					_bbar_items=_bbar_items.concat(['-',{
													text:'清除操作习惯',
													icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/clear.gif',
													handler:function(){
				...
				if(self.isShowCalculator){
					_bbar_items=_bbar_items.concat(['-',{
														text:'计算器',
														icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/cal.gif',
														handler:function(){
				...
				_bbar_items=_bbar_items.concat(['-',{
					text:'自动换行',
					icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/gridAutoRow.png',
					handler:function(){
	...
	clearConfigWin:function() {
		...
		if(gridConfigWin == null) { 
			gridConfigWin = new Ext.Window({
			    ...
			    buttons:[{
		            text:'确定',
		            icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/confirm.gif',
		          	handler:function() {
		            ...
		            text:'取消',
		            icon:TEMP_SYSTEM_WEB_ROOT+'/share/css/ext/img/close.gif',
		            handler:function(){
		            	gridConfigWin.hide();
		            }
		        }]
