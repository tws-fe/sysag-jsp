...
(function () {
	...
	/*
	 * e 返回值：ok、cancel、yes、no
	 * 
	 */
	matech.confirm=function(title,message,callback){
	    Ext.Msg.show({
	 	   title:title,  
	 	   msg:message,  
	 	   width:400,  
	 ...
	 * 
	 */
	matech.alert=function(message,callback){
	    Ext.Msg.show({
	 	   title:'提示消息',  
	 	   msg:message,  
	 	   width:400,  
	...
	matech.showWaiting=function(hight,wight,msg){
		...
		var strTalk="<div style=\"text-align:center;\"><img src=\"" + MATECH_SYSTEM_WEB_ROOT + "/share/css/ext/img/loading.gif\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><div><span style=\"font-size:16px;color: red\">"+msg+"</span></div>";
...
/***Grid处理相关函数****************************************************************/
	...
	matech.expExcel=function(tableId,_grid) {
		...
		var titleName = "";
		if(mainWindow.mainTab && mainWindow.mainTab.id=="homeIframe"){
			titleName=mainWindow.mainTab.getAttribute("menuname");
		}else{
...
/****表单操作*****************************************************************************************/
	...
	matech.setExtBtnShow=function(btnIndex,isEnabled){
		if(!Ext.isArray(btnIndex)){
			matech.alert("参数类型错误,第一个参数必须是数组！");
			return;
		}
		var index;
		var extObj;
	...
	matech.photoRender=function(fileType){
		...
		if(fileType=="0"){
    		s = '<div style="width:15px;height:15px">' +
    		 	'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/pap.png"></div>';
     		return s;
		}				 
		if(fileType.indexOf("xls")>-1){
			s = '<div style="width:15px;height:15px">' +
	    		'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/excel.png"></div>';
	     	return s;
		}
		if(fileType.indexOf("doc")>-1){
			s = '<div style="width:15px;height:15px">' +
    		 	'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/word.png"></div>';
     		return s;
		}
		if(fileType.indexOf("pdf")>-1){
			s = '<div style="width:15px;height:15px">' +
				'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/pdf.png"></div>';
 			return s;
		}
		if(fileType.indexOf("rar")>-1 || fileType.indexOf("zip")>-1){
			s = '<div style="width:15px;height:15px">' +
				'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/zip.png"></div>';
 			return s;
		}
		if(fileType.indexOf("jpg")>-1||fileType.indexOf("png")>-1||fileType.indexOf("gif")>-1){
			s = '<div style="width:15px;height:15px">' +
    		 	'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/pic.png"></div>';
     		return s;
		}else{
			s = '<div style="width:15px;height:15px">' +
    		 	'<img style="width:100%" src="'+MATECH_SYSTEM_WEB_ROOT + '/share/css/ext/img/ext/other.png"></div>';
     		return s;
		}		
	};
	/*
	...
	matech.callCamere = function(parent,_key,callback) {
		if(_key==""){
			matech.alert("请选择附件拍摄需要说明的对象，如底稿、疑点等！");
			return;
		}
	 ...
	 * 
	 */	
	matech.log=function(type,optFunc,optText,optSql,foreignid){
		if(!foreignid){
			foreignid="";
		}
		...
		$.ajax({
			cache: false,
			type: "POST",
			url:MATECH_SYSTEM_WEB_ROOT+"/general.do?method=clientLog",
			data:{type:type,optFunc:optFunc,optText:optText,optSql:optSql,foreignid:foreignid},
			async:true,
			error: function(request) {
				matech.alert("发送日志请求失败,请检查网络连接！");
			}
		});
	 ...
	 * 
	 */		
	matech.log.info=function(optFunc,optText,optSql,foreignid){
		matech.log("info",optFunc,optText,optSql,foreignid);
	};
	/*
	 * 
	 * 记录后台warn日志
	 ...
	 * 
	 */		
	matech.log.warn=function(optFunc,optText,optSql,foreignid){
		matech.log("warn",optFunc,optText,optSql,foreignid);
	};
	 ...
	 * 
	 */		
	matech.log.datebase=function(optFunc,optText,optSql,foreignid){
		matech.log("database",optFunc,optText,optSql,foreignid);
	};
	...
	matech.openAttach=function(handler,attachId,isIndex){
		...
		if(!parent.showAttach){
			matech.alert("找不到附件下载控件,请联系系统管理员！");
			return;
		}
		if(!handler || handler==""){
		...
		if(attachList.length>0){
			...
			matech.alert("没有对应对应的文档！");
		}					
		
	};	
	
	//判断IE版本
	matech.IEVersion=function() {
