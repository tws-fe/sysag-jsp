...
Ext.ux.tree.TreeGridNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
    ...
    renderElements : function(n, a, targetNode, bulkRender){
        ...
        var rowstyle=n.attributes.ROWSTYLE;
        if(rowstyle==null || rowstyle==""){
        	rowstyle="rowstyle";
        }
        var colstyle=n.attributes.COLSTYLE;
        if(rowstyle==null || rowstyle==""){
        	rowstyle="";
        }
        ...
        buf = [
             '<tbody class="x-tree-node" >',
                '<tr ext:tree-node-id="', n.id ,'" class="x-tree-node-el x-tree-node-leaf ','x-treegrid-row-',rowstyle, a.cls, '" >',
                    '<td class="x-treegrid-col',colstyle,'">',
        ...
        for(i = 1, len = cols.length; i < len; i++){
            c = cols[i];
            buf.push(
                    '<td class="x-treegrid-col',colstyle,' ', (c.cls ? c.cls : ''), '">',
...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.tree.TreePanel, {
        ...
        if(!this.internalTpl){                                
            this.internalTpl = new Ext.XTemplate(
                ...
                '<div class="x-treegrid-root-node">',
                    ...
                    '<table style="height:25px;"><tr><td></td></tr></table>',
                '</div>'
            );
        }
    ...
    setColumnVisible : function(index, visible) {
        ...
        if(this.columns[index].hiddenEvent){
        	this.columns[index].hiddenEvent(!visible);
        }
        this.updateColumnWidths();
    },
