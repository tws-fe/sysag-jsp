...
{   
        ...
        if (!ts.treeui) {
            ts.treeui = new Ext.Template(
                '<div class="ux-maximgb-tg-uiwrap" style="width: {wrap_width}px">',
                    '{elbow_line}',
                    '<div class="{folderCls}" style="left: {folderleft}px"></div>',
    ...
    renderCellTreeUI : function(record, store)
    {
        ...
        tpl_data.wrap_width = (depth + 1) * 40; 
        if (level > 0) {
            tpl_data.elbow_line = '';
            rec = record;
            left = 0;
        ...
        if (store.isLeafNode(record)) {
            if (store.hasNextSiblingNode(record)) {
                ...
                tpl_data.folderCls="ux-maximgb-tg-elbow-leaf";
            }
            else {
            ...
            if (store.isExpandedNode(record)) {
                if (store.hasNextSiblingNode(record)) {
                    ...
                    tpl_data.folderCls="ux-maximgb-tg-elbow-folderOpen";
                }else {
        ...
        tpl_data.folderleft=tpl_data.left+20;
