...
Ext.onReady(function(){
	 ...
	 _grid= new Ext.matech.grid.GridPanel({
			id:"gridId_caseMyReceiveVisitList",
			autoid:"js:gbag.casevisit.CaseMyReceiveVisitListGridJson",
			param:{flag:"${flag}"},		
			currentPage:1,
			singleSelect:true,
			columns:[
			        ...
			        {id:"VISITTIME",header:"来访时间",dataIndex:"VISITTIME",width:160,sortable:true,align:"left"},
			...
			fields:["UUID","NAME","VISITTIME","TELNUM","RESULT","VISITFOR","RECEIVECOPNAME","RECEIVEREPLY","REPLY","CASENUMBER","CASETYPE","CASENATURENAME","CASENAME","STATENAMES","AUDITDIRECTORNAME",
...
function add(){
	...
	var url = MATECH_SYSTEM_WEB_ROOT + "/caseVist.do?method=caseVisitEdit&editType=add";
	window.location = url;	
...
function edit() {
	...
	var url = MATECH_SYSTEM_WEB_ROOT + "/caseVist.do?method=caseVisitEdit&editType=update&uuid=" + value;
...
function remove(){
	...
	if (confirm("您确认要将删除当前选中的数据吗？")) {
		var url = MATECH_SYSTEM_WEB_ROOT + "/caseVist.do?method=caseVisitRemove&editType=delete&uuid=" + value;
		window.location.href = url;		
	}
