	...
	new ExtButtonPanel({
			...
			items:[{
	            ...
	            handler:function(){
		        	...
		        	$("#suuid").val(uuid);
		            $("#revicetor").val(visitname);
		            $("#phonenum").val(telnum);
		            $("#replys").val(reply);
		           
		            
		           /*  [来访人姓名]您好！您于【来访时间】咨询[事由]，现答复如下：[答复内容]！—[口岸派出所]  */
		            
		            
		            $("#smsreply").val("【"+visitname+"】您好！您于【"+visittime+"】咨询【"+visitfor+"】，现答复如下：【"+reply+"】！—【口岸派出所】");
		           /*  alert(reply);
		            alert(pky); */
		            if(pky=="1"){
		            	getDealWin();
...
function backList(){
	...
	var url;
	if(pkey=="1"){
	   url = "${pageContext.request.contextPath}/caseVist.do?method=myvisits";//来访登记列表（主办民警是本人的案件）
	}else if(pfl=="fl"){
		url = "${pageContext.request.contextPath}/caseVist.do?method=unCaseVisitList";//无主办民警来访列表（内勤分流）
	}
	else{
		url = "${pageContext.request.contextPath}/caseVist.do?method=caseMyReceiveVisit";//来访登记列表（接访民警是本人）
	}
			...
			<table align="center" border="0" class="polic_style">
				<tbody id="tbodyFieldList">
				    ...
				    <tr>
						...
						<td >
			                <input name="auditdirectors" size=23 type="text" id="auditdirectors"  maxLength=100 valuemustexist=false  autoid="js:gbag.ToReloAuditdirectorType"  refer="主办民警"  value = "${caseVisit.auditdirectors}" <c:if test="${flag ne 'nq'}">readOnly </c:if> /> 
...
<div id="dealDiv" style="display:none">
	...
	<input type="hidden" id="suuid" name="suuid"  value="">
	<input type="hidden" id="replys" name="replys"  value="">
		 <table>
		           <tr>
						<td><label>接收人：</label></td>
						<td>	
							<input style="height: 24px; width: 165px" id="revicetor"  name="revicetor" maxLength=100  value = ""/>
						</td>
						<td><label>联系电话：</label></td>
						<td>
							<input style="height: 24px; width: 165px" id="phonenum" name="phonenum"  maxLength=100 type="text"   value = ""  />
						</td>
				   </tr>	 
			       <tr>
						<td>
						
							<label ><span class="mustSpan">短信内容：</span></label>
						</td>
					</tr>
					<tr>
						<td colspan="6">
							<textarea onscroll="this.rows++;" style="overflow-y:visible; height: 200px; width: 99%"   id="smsreply"  name="smsreply"  maxLength=500 ></textarea>
						</td>
					</tr>
...
function saveFormDivsms(){
	
	var  suuid=$("#suuid").val();
	var revicetor=$("#revicetor").val();
	var phonenum=$("#phonenum").val();
	var smsreply=$("#smsreply").val();
    var replys=$("#replys").val();

	$.ajax({
		type :"Post",
		async:true,
		url : "${pageContext.request.contextPath}/caseVist.do?method=caseVisitSaveSms&sent=sms",
		data:{suuid:suuid,revicetor:revicetor,phonenum:phonenum,smsreply:smsreply,replys:replys},
		success : function(data) {
				...
				var t1 = window.setTimeout(function(){
					$.ajax({
						type :"Post",
						async:true,
						url : "${pageContext.request.contextPath}/caseVist.do?method=caseVisitSaveSmsResult",
						data:{MsgId:data},
