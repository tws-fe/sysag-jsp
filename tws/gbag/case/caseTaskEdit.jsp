...
function saveFormDiv(){
	...
	//保存
	var url = "${pageContext.request.contextPath}/case.do?method=sumcaseTaskSave&ismyCase=${ismyCase}";
	thisForm.action = url;
	thisForm.submit();
...
function summitFormDiv(){
	...
	//保存
	var url = "${pageContext.request.contextPath}/case.do?method=sumcaseTaskSave&summit=TZSH";
	thisForm.action = url;
	thisForm.submit();
			...
			<table align="center" border="0" class="polic_style" style="width:99%;">
				<tbody>
					...
					<tr>
						...
						<td>
							<input style="width: 160px; height: 24px" id="casestate"  name="casestate" maxLength=100  type="text" valuemustexist=true  type="text"  autoid="js:BasicDicImpl"  refer="案件状态"  value = "${case.state}" noinput=true />
						</td>
					</tr>
					<tr>
						<td>
						...
						<td colspan="6">
							<table style='text-align:center ;' align='center' cellspacing='1' cellpadding='3' border='0' class='listTable'  id = 'ga_case_task_table'> 
								...
								<tr>
									...
									<th rowspan=1 colspan=1  align='center' style='width: 25%'>任务内容 </th>
									<!-- <th rowspan=1 colspan=1  align='center' style='width: 20%'>处理结果</th> -->
									
									<th rowspan=1 colspan=1  align='center' style='width: 22%'>处理结果</th>
									<th rowspan=1 colspan=1  align='center' style='width: 8%'>办理人</th>
									<th rowspan=1 colspan=1  align='center' style='width: 17%'>办理时间</th>
									<th rowspan=1 colspan=1  align='center' style='width: 8%'>是否有材料</th>
								...
								<tbody id = 'ga_case_task' >		
									...
									<tr id="ga_case_task_row${rows.index}" >
										<td style=" height: 24px; width: 135px">
											...
											<img style='cursor:hand;<c:if test="${field.state eq ('3')||field.state eq ('4')}">display:none</c:if>' alt='点击删除' onclick="table_delete('ga_case_task_row${rows.index}');" src='${pageContext.request.contextPath}/img/menu/delete.png' >
											<!--  <input <c:if test="${field.state eq ('3')||field.state eq ('4')}">type="hidden"</c:if> <c:if test="${field.state eq ('-1')||field.state eq ('0')}">type="button"</c:if> onclick="table_delete('ga_case_task_row${rows.index}');" style="cursor:hand;" value="点击删除"/>-->
											</c:if>
										...
										<%-- <td>
											<textarea class="required" onscroll="this.rows++;" style="overflow-y:visible; height: 24px; width: 180px" onpropertychange='autoHeight(this);' oninput='autoHeight(this)'  id="taskcontent_row${rows.index}" name="taskcontent" maxLength=500 <c:if test="${field.state eq ('3')||field.state eq ('4')}">readOnly</c:if> >${field.taskcontent} </textarea>
										</td> --%>
										<td>
											...
											autoid="700"  refer="任务内容"     value = "${field.taskcontent}" checkinput="false" <c:if test="${field.state eq ('3')||field.state eq ('4')}">readOnly</c:if> />
											<input type="hidden" id="taskcontents_row${rows.index}" name="taskcontents"  value="${field.taskcontent}">
										</td>												
										<%-- <td>
											<textarea  onscroll="this.rows++;" style="overflow-y:visible; height: 24px; width: 135px" onpropertychange='autoHeight(this);' oninput='autoHeight(this)'  id="istakedown_row${rows.index}" name="istakedown" maxLength=500 <c:if test="${field.state eq ('3')||field.state eq ('4')}">readOnly</c:if> >${field.istakedown}</textarea>
										</td> --%>	
										<td>
											<textarea  onscroll="this.rows++;" style="overflow-y:visible; height: 24px; width: 135px" onpropertychange='autoHeight(this);' oninput='autoHeight(this)'  id="taskresult_row${rows.index}" name="taskresult" maxLength=500 <c:if test="${field.state eq ('3')||field.state eq ('4')}">readOnly</c:if> >${field.taskresult}</textarea>
										</td>	
										<td>
										...
										<td>
											<input class="required" style="height: 24px; width: 165px" id="handletime_row${rows.index}" name="handletime"  maxLength=100  type="text"  value = "${field.handletime}"  readOnly/>
										</td>
										<td>
											<input  style="height: 24px; width: 80px" id="ispaper_row${rows.index}" name="ispaper"  maxLength=100 type="text" valuemustexist=true 
											autoid="700"  refer="材料显示状态"    noinput=true  value = "${field.ispaper}" <c:if test="${field.state eq ('3')||field.state eq ('4')}">readOnly</c:if> />
										</td>
										<!--<td>
											<input  style="height: 24px; width: 165px" id="ispaper_row${rows.index}" name="ispaper"  maxLength=100  type="checkbox" <c:if test="${field.ispaper eq('0')}"> checked="checked"</c:if>  value = "${field.ispaper}"/>
										</td>	-->
										...
										<td>
											<input class="required" style="width: 100px; height: 24px" id="state_row${rows.index}"  name="state" maxLength=100  type="text" valuemustexist=true  type="text"  autoid="js:BasicDicImpl"  refer="任务显示状态"  readonly  value = "${field.state}" noinput=true />
										</td>
									
										<c:if test="${flag ne 'police'&& ismyCase ne 'yes'}">									
										<td>
											<input style="width: 160px; height: 24px" id="state1_row${rows.index}"  name="state1" maxLength=100  type="text" valuemustexist=true  type="text"  autoid="js:BasicDicImpl"  refer="任务确认状态"  value = "${field.state1}" noinput=true />
										</td>										
										</c:if>										
									</tr>	
									</c:forEach>		
...
function table_add(obj){
	...
	for(var index=0;index<items.length;index++){
		...
		//html=html+"<lable style=\"height: 30px; width: 120px\">点击删除</lable>";
		//html=html+"<input type=\"button\" onclick=\"table_delete('ga_case_task_row${rows.index}');\" style=\"overflow-y:visible;\" value=\"点击删除\"/>"
		...
		html=html+"</td>";
		/*html=html+"<td>";
		html=html+"	<textarea class=\"required\" onscroll=\"this.rows++;\" style=\"overflow-y:visible; height: 24px; width: 135px\" onpropertychange='autoHeight(this);' oninput='autoHeight(this)'  id=\"taskcontent_"+curRowId+"\" name=\"taskcontent\" maxLength=500 >"+item+"</textarea>";
		html=html+"</td>"; */
 		html=html+"<td>";
		html=html+"<input class=\"required\" style=\"width: 180px; height: 24px\" id=\"taskcontent_"+curRowId+"\"  name=\"taskcontent\" maxLength=100   valuemustexist=true checkinput=false type=\"text\"  autoid=\"700\"  refer=\"任务内容\" />";
		html=html+"</td>";
		/* html=html+"<td>";
		...
		html=html+"</td>";
		html=html+"<td>";
		html=html+"	<input class=\"required\" style=\"height: 24px; width: 165px\" id=\"handletime_"+curRowId+"\" name=\"handletime\"  maxLength=100  type=\"text\"  value = \"${curDateTime}\" readOnly />";
		html=html+"</td>";
		html=html+"<td>";
		html=html+"<input  style=\"height: 24px; width: 80px\" id=\"ispaper_"+curRowId+"\" name=\"ispaper\"  maxLength=100  valuemustexist=true  type=\"text\"  autoid=\"700\"  refer=\"材料显示状态\"  value =\"1\"  noinput=true/>";
		html=html+"</td>";
		html=html+"<td>";
		html=html+"<input class=\"required\" style=\"width: 100px; height: 24px\" id=\"state_"+curRowId+"\"  name=\"state\" maxLength=100   valuemustexist=true  type=\"text\"  autoid=\"700\"  refer=\"任务编制状态\"  value = \"0\" readOnly noinput=true/>";
		html=html+"</td>";
		...
		initCombox("ispaper_"+curRowId);//初始化下拉列表
...
function table_delete(obj){
	//alert($("#"+obj));
	//"table_delete('ga_case_task_row${rows.index}');"
