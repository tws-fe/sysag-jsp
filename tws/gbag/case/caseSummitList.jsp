...
Ext.onReady(function(){
	...
	if("${flag}"=="police"||"${ismyCase}"=="yes"){
	 ...
	 _grid= new Ext.matech.grid.GridPanel({
			id:"gridId_caseSummitList",
			autoid:"js:gbag.casese.CaseSummitGridJson",
...
function _restoreSelectCheck(store,records){
	...
	store.each(function(r){
		...
		if(remindersum>=3){
			Ext.getCmp('gridId_caseSummitList').getView().getRow(girdcount).style.backgroundColor='red';
		}else if(remindersum>="1"&&remindersum<"3"){
			Ext.getCmp('gridId_caseSummitList').getView().getRow(girdcount).style.backgroundColor='orange';
		}
