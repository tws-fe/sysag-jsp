...
Ext.onReady(function(){
	...
	if("${flag}"=="police"||"${ismyCase}"=="yes"){
	 ...
	 _grid= new Ext.matech.grid.GridPanel({
			...
			autoid:"js:gbag.casese.CaseUnSummitGridJson",
...
function getTask(){
	...
	if (value == "") {
		...
		var url = contextPath + "/case.do?method=caseTaskEdit&editType=update&uuid="+ value+"&flag="+ flag+"&ismyCase="+ ismyCase;
...
function _restoreSelectCheck(store,records){
	...
	store.each(function(r){
		...
		if(remindersum>=3){
			Ext.getCmp('gridId_caseUnSummitList').getView().getRow(girdcount).style.backgroundColor='red';
		}else if(remindersum>="1"&&remindersum<"3"){
			Ext.getCmp('gridId_caseUnSummitList').getView().getRow(girdcount).style.backgroundColor='orange';
		}
