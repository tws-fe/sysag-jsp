...
Ext.onReady(function(){
	...
	if("${flag}"=="police"){
	 ...
	 _grid= new Ext.matech.grid.GridPanel({
			id:"gridId_caseUnHandOverList",
			autoid:"js:gbag.casese.CaseUnHandOverGridJson",
			param:{flag:"${flag}"},		
			currentPage:1,
			singleSelect:true,
			columns:[
...
function dealCase(dealType){
	...
	if (value == "") {
		...
		if(dealType=="2"){
			...
			if (confirm("您确认对当前案件进行催交吗？")) {
				var url = contextPath + "/case.do?method=caseDeal&r=" + Math.random();
				var request = "&editType=deal&types=cj&dealType="+dealType+"&uuid=" + value;
				var result = ajaxLoadPageSynch(url, request);
				if(result.indexOf("ok")>-1){
					alert("该案件已催交！");
					window.location.reload();
