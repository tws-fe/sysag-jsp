...
Ext.onReady(function(){
	if("${flag}"=="police"){
	 ...
	 _grid= new Ext.matech.grid.GridPanel({
			id:"gridId_caseTaskDealList",
			autoid:"js:gbag.casese.CaseTaskDealGridJson",
			param:{flag:"${flag}"},		
			currentPage:1,
			singleSelect:true,
			columns:[
				...
				fields:["UUID","ISFINISH","REMINDERSUM","TASKCONTENT","HANDLETIME","TASKRESULT","CASENUMBER","TASKRESULT","CASENATURENAME","CASENAME","CASENATURENAME","CASETYPE","STATENAMES","_USERNAME_AUDITDIRECTOR",
...
function _restoreSelectCheck(store,records){
	...
	store.each(function(r){
		...
		if(remindersum>=3){
			Ext.getCmp('gridId_caseTaskDealList').getView().getRow(girdcount).style.backgroundColor='red';
		}else if(remindersum>="1"&&remindersum<"3"){
			Ext.getCmp('gridId_caseTaskDealList').getView().getRow(girdcount).style.backgroundColor='orange';
		}
