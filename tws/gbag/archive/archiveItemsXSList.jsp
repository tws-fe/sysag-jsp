...
Ext.onReady(function(){
			 ...
			 items:[   	
			        {
			   			...
			   			handler:function () {	
			   				var url = MATECH_SYSTEM_WEB_ROOT + "/police.do?method=archiveTaskItemsList&savastyle=XS";
	 ...
	 _grid= new Ext.matech.grid.GridPanel({
			id:"gridId_archiveItemsXSList",
			autoid:"js:gbag.archive.archiveItemsXSListGridJson",
...
//列表式新增
function addlist(){
	var url = MATECH_SYSTEM_WEB_ROOT + "/police.do?method=archiveItemsXS&savestyle=XS&editType=add&show=list";
	window.location = url;	
}
//列表编辑
function editlist() {
	...
	var url = MATECH_SYSTEM_WEB_ROOT + "/police.do?method=archiveItemsXSEdit&show=list&savestyle=XS&editType=update&uuid=" + value;
...
function remove() {
	...
	if (confirm("您确认要将删除当前选中的档案材料吗？")) {
		var url = MATECH_SYSTEM_WEB_ROOT + "/police.do?method=XSeditRemove&editType=remove&uuid=" + value;
		window.location.href = url;	
	}
...
function deliver() {
	...
	if (confirm("您确认要将移交当前选中的档案材料吗？")) {
		var url = MATECH_SYSTEM_WEB_ROOT + "/police.do?method=deliver&delivertype=xs&uuid=" + value;
		window.location.href = url;	
	}
