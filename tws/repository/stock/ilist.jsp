...
function ext_init(){  
	...
	_grid= new Ext.matech.grid.GridPanel({
		id:"gridId_unitList",
		autoid:"js:repository.stock.InputStockListGridJson",
		param:{unitId:"${unitId}"},
		currentPage:1,
		singleSelect:true,
	
		columns:[{id:"INPUTDATE",header:"入库日期",dataIndex:"INPUTDATE",width:120,sortable:true,align:"left"},
	        	{id:"INPUTTYPE",header:"入库类型",dataIndex:"INPUTTYPE",width:330,sortable:true,align:"left"},
					...
					{id:"BELONGCONTRACT",header:"所属合同",dataIndex:"BELONGCONTRACT",width:120,sortable:true,align:"left"},
		...
		fields:["UUID","INPUTDATE","INPUTTYPE","PRODUCTNUMBER","NUMBER","BELONGCONTRACT","REGISTRANT","REASON","ORGANID_"],
		ondbclick:edit,
		region: 'center'
	});	
...
function add() {
	...
	window.location = "${ctx}/inputStock.do?method=edit&editType=add&unitId=${unitId}";
}
...
function edit() {
 	...
 	var stockId = _grid.chooseValue("UUID");
   	...
   	window.location = "${ctx}/inputStock.do?method=edit&editType=update&stockId="+stockId+"&unitId=${unitId}";
	...
	if(stockId=="") {
		...
		if(matech.checkIsExist("js:dept.DeptHasUser",stockId,true)) {
			...
			if(!confirm("确定要删除该对象？")) {
				...
				matech.ajaxSumit("${ctx}/inputStock.do?method=delete","unitId=${unitId}&stockId="+stockId,true,function(){
						_grid.goSearch();
				});
			}
		}
